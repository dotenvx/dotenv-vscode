...
const javascriptEnv = require('./javascript-env')

const javascriptHover = {
  provideHover: function (document, position, token) {
    const regex = javascriptEnv.reference(document, '([A-Za-z_$][\\w$]*)', 'g')
    if (regex) {
      for (const match of document.lineAt(position).text.matchAll(regex)) {
        const key = match[1]
        const start = match.index + match[0].length - key.length
...
const csharpHover = dotnet.hover
...
const javascriptCompletion = {
  provideCompletionItems: function (document, position) {
    ...
    if (!linePrefix.endsWith('process.env.') && !linePrefix.endsWith('import.meta.env.') &&
        !javascriptEnv.reference(document, '$')?.test(linePrefix)) {
      return undefined
    }
...
const csharpCompletion = dotnet.completion
