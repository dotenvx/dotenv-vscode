...
function resolveCustomTextEditor (document, panel, context) {
  ...
  const send = (extra = {}) => {
    if (client && !disposed) webview.postMessage({ type: 'document', client, text: text(), version: document.version, filename: path.basename(document.uri.fsPath || ''), options: options(), ...extra })
  }
  ...
  const subscriptions = [
    ...
    vscode.workspace.onDidChangeConfiguration(event => { if (event.affectsConfiguration('editor', document.uri)) send() }),
