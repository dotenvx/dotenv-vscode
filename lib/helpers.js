...
function displayValue (value) {
  return (settings.secretpeekingEnabled() ? value : _partialMask(value)) || '(empty)'
}

function valueDocumentation (values) {
  const doc = new vscode.MarkdownString()
  for (const { source, value } of values) {
    doc.appendText(source)
    doc.appendMarkdown('\n\n')
    doc.appendText(displayValue(value))
    doc.appendMarkdown('\n\n---\n\n')
  }
  return doc
}
...
function valueHover (key, document, range) {
  ...
  return new vscode.Hover(values.every(entry => entry.value === values[0].value) ? displayValue(values[0].value) : valueDocumentation(values), range)
}
...
function hover (language, document, position) {
  ...
  if (!matches) {
    ...
    if (position.character >= start && position.character <= end) {
      return valueHover(key, document)
    } else {
...
function _partialMask (str) {
  ...
  const maskChars = settings.cloakIcon().repeat(Math.max(0, str.length - 2))
...
function autocomplete (triggerCharacter, document, position) {
  ...
  return entries.map(function (env) {
    ...
    const completionItemLabel = {
      label: key,
      detail: values.every(entry => entry.value === values[0].value) ? ` ${displayValue(values[0].value)}` : ` (${values.length} files)`,
    ...
    item.documentation = valueDocumentation(values)
