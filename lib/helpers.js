...
function displayValue (value) {
  return (settings.secretpeekingEnabled() ? value : _mask(value)) || '(empty)'
}

function valueDocumentation (values, batch, key) {
  ...
  for (const { source, value } of values) {
    ...
    doc.appendText(batch ? ((revealed ? value : _mask(value)) || '(empty)') : displayValue(value))
