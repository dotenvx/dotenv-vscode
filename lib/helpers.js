...
function displayValue (value, revealed, uri) {
  return (revealed ? value : _mask(value, uri)) || '(empty)'
}
...
function _mask (str, uri) {
  return settings.cloakIcon(uri).repeat(str.length)
}
...
function autocomplete (triggerCharacter, document, position) {
  ...
  const items = entries.map(function (env) {
    ...
    const completionItemLabel = {
      label: key,
      detail: values.every(entry => entry.value === values[0].value) ? ` ${_mask(values[0].value, document.uri) || '(empty)'}` : ` (${values.length} files)`,
    ...
    item.documentation = valueDocumentation(values, batch, key, undefined, document.uri)

    return item
  })
  const valuesByKey = new Map(entries)
  return completionReveal.track(batch, items, (key, refreshed) => valueDocumentation(valuesByKey.get(key), refreshed, key, undefined, document.uri))
}
