...
function valueHover (key, document, range) {
  ...
  const contents = [values.every(entry => entry.value === values[0].value) ? displayValue(values[0].value, revealed) : valueDocumentation(values, undefined, undefined, revealed)]
